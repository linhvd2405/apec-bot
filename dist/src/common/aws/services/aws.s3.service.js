...
let AwsS3Service = class AwsS3Service {
    ...
    async listItemInBucket(prefix) {
        ...
        try {
            ...
            const mapList = listItems.Contents.map((val) => {
                ...
                const filename = val.Key.substring(lastIndex, val.Key.length);
                const mime = filename
    ...
    async putItemInBucket(filename, content, options) {
        let path = options && options.path ? options.path : undefined;
    ...
    async createMultiPart(filename, options) {
        ...
        const multiPartCommand = new client_s3_1.CreateMultipartUploadCommand(multiPartInput);
        try {
    ...
    async uploadPart(path, content, uploadId, partNumber) {
        ...
        const uploadPartCommand = new client_s3_1.UploadPartCommand(uploadPartInput);
        try {
    ...
    async completeMultipart(path, uploadId, parts) {
        ...
        const completeMultipartCommand = new client_s3_1.CompleteMultipartUploadCommand(completeMultipartInput);
        try {
    ...
    async abortMultipart(path, uploadId) {
        ...
        const abortMultipartCommand = new client_s3_1.AbortMultipartUploadCommand(abortMultipartInput);
        try {
