...
let FileSizeImagePipe = class FileSizeImagePipe {
    ...
    async validate(size) {
        const maxSizeInBytes = this.request.__customFileSize
            ...
            : this.configService.get('file.image.maxFileSize');
...
let FileSizeExcelPipe = class FileSizeExcelPipe {
    ...
    async validate(size) {
        const maxSizeInBytes = this.request.__customFileSize
            ? this.helperFileService.convertToBytes(this.request.__customFileSize)
            : this.configService.get('file.excel.maxFileSize');
        if (size > maxSizeInBytes) {
            throw new common_1.PayloadTooLargeException({
                statusCode: file_status_code_constant_1.ENUM_FILE_STATUS_CODE_ERROR.FILE_MAX_SIZE_ERROR,
                message: 'file.error.maxSize',
...
let FileSizeVideoPipe = class FileSizeVideoPipe {
    ...
    async validate(size) {
        const maxSizeInBytes = this.request.__customFileSize
            ? this.helperFileService.convertToBytes(this.request.__customFileSize)
            : this.configService.get('file.video.maxFileSize');
        if (size > maxSizeInBytes) {
            throw new common_1.PayloadTooLargeException({
                statusCode: file_status_code_constant_1.ENUM_FILE_STATUS_CODE_ERROR.FILE_MAX_SIZE_ERROR,
                message: 'file.error.maxSize',
...
let FileSizeAudioPipe = class FileSizeAudioPipe {
    ...
    async validate(size) {
        const maxSizeInBytes = this.request.__customFileSize
            ? this.helperFileService.convertToBytes(this.request.__customFileSize)
            : this.configService.get('file.audio.maxFileSize');
        if (size > maxSizeInBytes) {
            throw new common_1.PayloadTooLargeException({
                statusCode: file_status_code_constant_1.ENUM_FILE_STATUS_CODE_ERROR.FILE_MAX_SIZE_ERROR,
                message: 'file.error.maxSize',
