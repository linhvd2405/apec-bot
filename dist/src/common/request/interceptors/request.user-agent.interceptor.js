    ...
    async intercept(context, next) {
        if (context.getType() === 'http') {
            const request = context
                .switchToHttp()
                .getRequest();
            const os = this.configService.get('middleware.userAgent.os');
            const browser = this.configService.get('middleware.userAgent.browser');
            ...
            if (!browser.some((val) => val.match(new RegExp(userAgent.browser.name)))) {
                throw new common_1.ForbiddenException({
